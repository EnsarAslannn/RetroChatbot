...
| `USER_DAILY_CHAT_LIMIT` | Oturumlu kullanıcı başına günlük model isteği | `200` |
| `REDIS_URL` | Birden fazla örnek arasında ortak istek sınırı için Redis bağlantısı | Boş; SQLite kullanılır |
| `METRICS_TOKEN` | `/api/metrics` Bearer erişim belirteci | Yok; uç nokta kilitli |

Üretimde HTTPS arkasında `uvicorn app.main:app --host 0.0.0.0 --port 8000` komutuyla çalıştırın; `AUTH_COOKIE_SECURE=true`, güçlü bir `METRICS_TOKEN` ve birden fazla uygulama örneğinde ortak `REDIS_URL` ayarlayın. SQLite dosyasını kalıcı diskte tutun ve yedekleyin. İstemcide durdurulan bir istek, sağlayıcıya ulaşmışsa kullanım maliyeti doğurabilir.
